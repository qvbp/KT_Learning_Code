...
class CAKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, kernel_size1=1, kernel_size2=3,
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    d_model=d_model, d_feature=d_model / num_attn_heads,
                                  kernel_size1=kernel_size1, kernel_size2=kernel_size2,  kq_same=self.kq_same, model_type=self.model_type, emb_type=self.emb_type)
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        # Batch First
        if emb_type.startswith("qid"):
            # q_data就是论文中的ct  target就是论文中的rt
            # cct和e(ct,rt)
            q_embed_data, qa_embed_data = self.base_emb(q_data, target)  # target是 一个respond
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
                 kernel_size1, kernel_size2, n_heads, dropout, kq_same, model_type, emb_type):
        super().__init__()
        """
        ...
        if model_type in {'cakt'}:
            self.blocks_1 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 kernel_size1=kernel_size1, kernel_size2=kernel_size2, dropout=dropout, n_heads=n_heads, kq_same=kq_same, emb_type=emb_type)
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
                 kernel_size1, kernel_size2, n_heads, dropout,  kq_same, emb_type):
        super().__init__()
        """
        ...
        self.ICB = ICB(d_model, d_model, kernel_size1, kernel_size2)
    ...
    def forward(self, mask, query, key, values, apply_pos=True, pdiff=None):
        ...
        if apply_pos:
            ...
            query2 = self.ICB(query)  # ICB替代FFN
...
# 该模块生成并学习位置嵌入，它们与输入序列的每个位置对应。
# 在 Transformer 等模型中，位置嵌入通常会与输入嵌入相加，以便为模型提供位置信息，从而更好地处理序列数据
